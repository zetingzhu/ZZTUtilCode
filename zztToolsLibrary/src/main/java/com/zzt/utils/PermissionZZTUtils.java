 ...
 * 申请权限工具
 */
public class PermissionZZTUtils {
    // 申请日历权限
    ...
    private static volatile PermissionZZTUtils instance;

    public PermissionZZTUtils() {
    }

    public static PermissionZZTUtils getInstance() {
        if (instance == null) {
            synchronized (PermissionZZTUtils.class) {
                if (instance == null) {
                    instance = new PermissionZZTUtils();
                }
            }
        }
        return instance;
     ...
     *
     * @param request 是否立即请求当前权限
     */
    public boolean checkPermission(Activity mActivity, String[] permissions, PermissionListener permissionListener, Boolean request) {
