...
public class QMUIFloatLayout extends ViewGroup {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mAverageCount = array.getInt(R.styleable.QMUIFloatLayout_qmui_average_count, 0);
        array.recycle();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthSpecMode == MeasureSpec.EXACTLY) {
            ...
            //  设置最小宽度
            if (mAverageCount > 0) {
                minWidth = (resultWidth - getPaddingLeft() - getPaddingRight() - (mChildHorizontalSpacing * (mAverageCount - 1))) / mAverageCount;
            } else {
                minWidth = 0;
            }
            ...
            for (int i = 0; i < count; i++) {
                ...
                final int childWidthMeasureSpec = getChildMeasureSpec(widthMeasureSpec, getPaddingLeft() + getPaddingRight(), childLayoutParams.width);
                final int childHeightMeasureSpec = getChildMeasureSpec(heightMeasureSpec, getPaddingTop() + getPaddingBottom(), childLayoutParams.height);
                child.measure(childWidthMeasureSpec, childHeightMeasureSpec);


                final int childw = Math.max(minWidth, child.getMeasuredWidth());
